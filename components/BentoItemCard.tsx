...
import { FigmaPlaceholderIcon } from './icons/IconComponents'; // Default/placeholder icon

export const BentoItemCard: React.FC<{ item: BentoItem }> = ({ item }) => {
  const baseCardClass = "shadow-[0px_2px_4px_rgba(0,0,0,0.04)] rounded-[8px] outline outline-1 outline-[rgba(0,0,0,0.06)] outline-offset-[-1px] transition-all hover:shadow-lg";
  ...
  if (item.variant === 'smol_icon_text_vertical') {
    return (
      ...
      >
        {item.icon && (
          <div className={`p-[8px] ${item.iconBgClass || 'bg-[#3E3E3E]'} rounded-[6px] w-[40px] h-[40px] flex items-center justify-center shadow-[0px_2px_3px_rgba(0,0,0,0.08)]`}>
            <item.icon className="w-[24px] h-[24px] text-white" />
          </div>
        )}
        <div className="flex flex-col gap-[8px]">
          {item.title && <h3 className="text-black text-[16px] font-medium leading-[20px]">{item.title}</h3>}
          {item.description && <p className="text-[rgba(0,0,0,0.48)] text-[10px] leading-[12px]">{item.description}</p>}
        </div>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="flex flex-col justify-start gap-[12px] w-[140px] shrink-0">
          {item.icon && (
            <div className={`p-[6px] ${item.iconBgClass || 'bg-[#3E3E3E]'} rounded-[6px] w-[40px] h-[40px] flex items-center justify-center shadow-[0px_2px_3px_rgba(0,0,0,0.08)]`}>
              <item.icon className="w-[24px] h-[24px] text-white" />
            </div>
          )}
          <div className="flex flex-col gap-[8px] flex-grow">
            {item.title && <h3 className="text-black text-[14px] leading-[16px]">{item.title}</h3>}
            {item.description && <p className="text-[rgba(0,0,0,0.48)] text-[12px] leading-[16px]">{item.description}</p>}
          </div>
        </div>
        {item.imageUrl && (
          <img src={item.imageUrl} alt={item.imageAlt || item.title || 'Showcase image'} className="w-[155px] h-[147px] rounded-[6px] object-cover ml-[24px]" />
        )}
      </div>
    );
  }
  ...
  if (item.variant === 'big_text_over_image') {
    return (
      ...
      >
        ...
        {item.imageUrl && (
          <img src={item.imageUrl} alt={item.imageAlt || item.title || 'Showcase image'} className="w-full h-[192px] rounded-[6px] object-cover mt-[24px]" />
        )}
      </div>
    );
  }
