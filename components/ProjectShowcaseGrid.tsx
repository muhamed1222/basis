...
export const ProjectShowcaseGrid: React.FC = () => {
  ...
  return (
    <div className="w-full md:max-w-[820px] space-y-[24px]"> {/* Width from design */}
      <div>
        <h2 className="text-black text-[18px] font-semibold leading-[24px] mb-[20px] px-[12px] py-[12px] bg-white rounded-[8px]"> {/* Styling from design's title bar */}
          Project Showcase
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px] items-start">
          ...
          <div className="space-y-[24px]">
             {showcaseDataTop.find(item => item.id === 'proj1') && 
              <BentoItemCard item={showcaseDataTop.find(item => item.id === 'proj1')!} />}
             {showcaseDataTop.find(item => item.id === 'proj3') && 
              <BentoItemCard item={showcaseDataTop.find(item => item.id === 'proj3')!} />}
          </div>
          <div className="space-y-[24px]">
            {showcaseDataTop.find(item => item.id === 'proj2') && 
              <BentoItemCard item={showcaseDataTop.find(item => item.id === 'proj2')!} />}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-[24px] items-start"> {/* Based on 3 cards in this row in design */}
