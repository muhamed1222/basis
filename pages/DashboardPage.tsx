...
const ProjectCardPlaceholder: React.FC<{ title: string; lastUpdated: string }> = ({ title, lastUpdated }) => (
  <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow p-4">
    <h3 className="text-lg font-semibold font-pragmatica text-gray-800">{title}</h3>
    <p className="text-sm text-gray-500 mb-3">Последнее обновление: {lastUpdated}</p>
    <div className="space-x-2">
      <Link to="/editor" state={{ projectId: title }} className="text-sm text-indigo-600 hover:text-indigo-800">Редактировать</Link>
    ...
    <StandardPageLayout title="3. Dashboard">
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-gray-200">
            <section>
                <h2 className="text-xl font-semibold font-pragmatica mb-3 text-gray-700">Быстрые настройки</h2>
                <ul className="space-y-2">
                    <li><Link to="/account" className="text-indigo-600 hover:underline">Настройки аккаунта</Link></li>
                    <li><Link to="/account#security" className="text-indigo-600 hover:underline">Безопасность</Link></li>
                </ul>
            </section>
            <section>
                <h2 className="text-xl font-semibold font-pragmatica mb-3 text-gray-700">Тарифы и Подписка</h2>
                 <p className="text-sm text-gray-600 mb-2">Ваш текущий тариф: <span className="font-semibold text-green-600">Free</span></p>
                <Link to="/billing" className="text-indigo-600 hover:underline">Управление подпиской и тарифами</Link>
            </section>
        </div>
