...
const AdminSectionLink: React.FC<{ to: string; title: string; description: string }> = ({ to, title, description }) => (
  <Link to={to} className="block bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
    <h3 className="text-lg font-semibold font-pragmatica text-indigo-700 mb-1">{title}</h3>
  ...
  if (!isStaff) {
    return (
      <StandardPageLayout title="Access Denied">
        ...
        <Link to="/" className="text-indigo-600 hover:underline mt-4 inline-block">Вернуться на главную</Link>
      </StandardPageLayout>
    );
  }
  ...
  return (
    <StandardPageLayout title="10. Admin Panel">
      <div className="space-y-6">
        <p className="text-sm text-gray-500">Добро пожаловать в административную панель. Здесь вы можете управлять платформой.</p>
