...
Контекст `AuthContext` предоставляет функции `login`, `signup` и `logout` и хранит текущего пользователя. Формы на странице `AuthPage` используют эти методы. Пока реализованы только заглушки вызовов API.
